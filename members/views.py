...
from .models import UserProfile, PointsDetails
import logging
...
def post_view(request):
    ...
    post_count_today = PointsDetails.objects.filter(
        user=user,
        action_type='post',
        actioned_at__date=today
    ).count()

    logger.info(f"Post count today for user {user.username}: {post_count_today}")

    if post_count_today >= 5:
        logger.info(f"User {user.username} has reached the daily post limit. No points will be awarded.")
        # 不進行任何操作
...
def add_points(user, action_type, points):
    ...
    user_profile.save()
    logger.info(f"User {user.username} has been awarded {points} points for action {action_type}. Total points: {user_profile.points}")
