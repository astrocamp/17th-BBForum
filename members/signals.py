...
def update_user_points(sender, instance, **kwargs):
    ...
    if instance.action_type == "login":
        ...
        # 检查用户今天是否已经登录过
        if not PointsDetails.objects.filter(
            user=instance.user, action_type="login", actioned_at__date=today
        ).exists():
        ...
        # 检查当天的发文次数
        today = timezone.now().date()
        post_count = PointsDetails.objects.filter(
            user=instance.user, action_type="post", actioned_at__date=today
        ).count()
        if post_count < 5:
            user_profile.points += instance.point_number
            user_profile.save()
    ...
    PointLog.objects.create(
        ...
        points_change=instance.point_number,
    )
