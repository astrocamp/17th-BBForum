...
class PointSystemTestCase(TestCase):
    ...
    def test_login_points(self):
        ...
        PointsDetails.objects.create(
            user=self.user, action_type="login", point_number=1
        )
    ...
    def test_post_points(self):
        ...
        for i in range(6):
            PointsDetails.objects.create(
                user=self.user, action_type="post", point_number=2
            )
    ...
    def test_like_received_points(self):
        ...
        for i in range(100):
            PointsDetails.objects.create(
                user=self.user, action_type="like_received", point_number=1
            )
    ...
    def test_reported_points(self):
        ...
        # 測試被檢舉
        PointsDetails.objects.create(
            user=self.user, action_type="reported", point_number=-2
        )
